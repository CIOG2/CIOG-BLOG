...
import StickerMessage from '../StickerMessage/index';

// ── SVG Icons ────────────────────────────────────────────────
const ArrowSvg = () => (
    <svg viewBox="0 0 8 13" height="13" width="8" preserveAspectRatio="xMidYMid meet" version="1.1" x="0px" y="0px" enable-background="new 0 0 8 13">
...
interface MessageProps {
    ...
    contentType?: 'text' | 'audio' | 'sticker' | 'image';
    audioSrc?: string;
    audioLabel?: string;
    audioDuration?: string;
    audioAvatarSrc?: string;
    audioAvatarAlt?: string;
    audioAccentColor?: string;
    stickerSrc?: string;
    stickerAlt?: string;
    imageSrc?: string;
    imageAlt?: string;
...
const Message: FunctionComponent<MessageProps> = ({
    ...
    const isAudioMessage   = contentType === 'audio' && audioSrc;
    const isStickerMessage = contentType === 'sticker' && stickerSrc;
    const isImageMessage   = contentType === 'image' && imageSrc;
    const hasText          = typeof text === 'string' && text.trim().length > 0;
    const hasStatus        = status && ['sent', 'received', 'read'].includes(status);
    const showMetadata     = !isAudioMessage && Boolean(time || hasStatus);

    const containerClasses = [
        'chat__message--container',
        `chat__message--container-${type}`,
        consecutive && 'chat__message--container-consecutive',
        isStickerMessage && 'chat__message--container-is-sticker',
        interactive && 'chat__message--interactive',
    ].filter(Boolean).join(' ');

    const statusClasses = [
        'chat__message--status',
        status && `chat__message--status-${status}`,
    ].filter(Boolean).join(' ');

    const interactiveStyles = interactive
        ? { opacity: 0, display: 'none', transform: 'translateY(10px)' }
        : undefined;
    ...
    return (
        ...
        >
            <div class={`chat__message ${hasText ? 'chat__message--has-text' : ''}`}>
                {!consecutive && !isStickerMessage && (
                    <div class="chat__message--arrow">
                        <ArrowSvg />
                    </div>
                )}

                {isStickerMessage && (
                    <div class="chat__message--sticker">
                        <StickerMessage src={stickerSrc!} alt={stickerAlt} />
                    </div>
                )}

                {isAudioMessage && (
                    <div class="chat__message--audio">
                        <AudioMessage
                            src={audioSrc!}
                            durationHint={audioDuration}
                            messageTime={time}
                            status={status}
                            avatarSrc={audioAvatarSrc}
                            avatarAlt={audioAvatarAlt}
                ...
                {isImageMessage && (
                    <div class="chat__message--image">
                        <ImageMessage src={imageSrc!} alt={imageAlt} />
                    </div>
                )}
                ...
                {hasText && (
                    <p class="chat__message--text">
                        {text}
                        <span class="chat__message--spacer" />
                    </p>
                )}

                {showMetadata && (
                    <div class="chat__message--elements-container">
                        {time && <span class="chat__message--time">{time}</span>}
                        {hasStatus && (
                            <div class={statusClasses}>
                                {(status === 'read' || status === 'received') && <DoubleCheckSvg />}
                                {status === 'sent' && <SingleCheckSvg />}
                            </div>
                        )}
                    </div>
                )}
